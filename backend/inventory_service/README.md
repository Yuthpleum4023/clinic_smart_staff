...
## Server-owned inventory item code

Inventory item codes are generated by `inventory_service`, not by Flutter or
other clients.

- Allocation is scoped by authenticated `clinicId`.
- Current internal format is `INV-000001`, `INV-000002`, ...
- The existing `sku` field stores this server-generated internal item code.
- Item creation rejects client-supplied `sku`.
- `currentQty` remains server-owned and starts at `0`.
- Stock changes occur only through the movement ledger.
- A future manufacturer barcode / supplier product code must use a separate
  field instead of overloading the internal item code.
